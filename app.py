...
GITHUB_REPO = st.secrets.get("GITHUB_REPO", "")  # 예: "i088785/umbrella-rental"

def load_data_from_github(filename):
    """GitHub에서 최신 CSV 파일을 불러옵니다."""
    if not GITHUB_TOKEN or not GITHUB_REPO:
        # Secrets가 없으면 로컬 파일 시도
    ...
    if response.status_code == 200:
        ...
        if not content.strip():
            return []
        df = pd.read_csv(StringIO(content), dtype={"학번": str, "전화번호": str})
        for column in ["대여일", "반납예정일", "반납처리일", "경고일"]:
            if column in df.columns:
                df[column] = pd.to_datetime(df[column], errors='coerce').dt.date
        return df.to_dict("records")
...
def save_data_to_github(filename, data):
    """GitHub 저장소에 직접 CSV 파일을 업로드(커밋)합니다."""
    df = pd.DataFrame(data)
    for col in df.columns:
        if df[col].dtype == 'object':
            df[col] = df[col].apply(lambda x: x.strftime("%Y-%m-%d") if isinstance(x, (date, datetime)) else x)
    
    csv_content = df.to_csv(index=False, encoding="utf-8-sig")
    
    if not GITHUB_TOKEN or not GITHUB_REPO:
        # Secrets가 없으면 로컬에만 저장
    ...
    payload = {
        "message": f"Update {filename} via Streamlit App",
...
def add_warning(rental, overdue_days):
    ...
    save_data_to_github(WARNING_FILE, st.session_state.warning_logs)
...
if "rentals" not in st.session_state:
    st.session_state.rentals = load_data_from_github(RENTAL_FILE)

if "return_logs" not in st.session_state:
    st.session_state.return_logs = load_data_from_github(RETURN_FILE)

if "warning_logs" not in st.session_state:
    st.session_state.warning_logs = load_data_from_github(WARNING_FILE)
...
rental_text = st.text_input("해당 신청 문구", placeholder="예: 학번/이름/전화번호/우산번호/대여날짜")
...
if st.button("대여자 정보 등록", type="primary"):
    try:
        ...
        if warning_count >= 2:
            ...
            save_data_to_github(RENTAL_FILE, st.session_state.rentals)
...
rented_umbrellas = [rental["우산번호"] for rental in st.session_state.rentals]
available_umbrellas = [u for u in UMBRELLAS if u not in rented_umbrellas]
        ...
        for index, rental in enumerate(st.session_state.rentals):
            ...
            with st.container(border=True):
                if st.session_state.edit_index == index:
                    ...
                    with edit_col1:
                        new_student_id = st.text_input("학번", value=str(rental["학번"]), key=f"edit_student_id_{index}")
                        new_name = st.text_input("이름", value=rental["이름"], key=f"edit_name_{index}")
                        new_phone = st.text_input("전화번호", value=str(rental["전화번호"]), key=f"edit_phone_{index}")
                    with edit_col2:
                    ...
                    if save_col.button("수정저장", type="primary", key=f"save_edit_{index}"):
                        other_rented_umbrellas = [item["우산번호"] for i, item in enumerate(st.session_state.rentals) if i != index]
                    ...
                    st.write(f"대여일: {rental['대여일'].strftime('%Y.%m.%d') if isinstance(rental['대여일'], date) else rental['대여일']}")
                    st.write(f"반납예정일: {rental['반납예정일'].strftime('%Y.%m.%d') if isinstance(rental['반납예정일'], date) else rental['반납예정일']}")
                    ...
                    if btn_col2.button("반납완료", key=f"return_{index}"):
                        ...
                        save_data_to_github(RENTAL_FILE, st.session_state.rentals)
                        save_data_to_github(RETURN_FILE, st.session_state.return_logs)
                        st.rerun()

with right:
    st.subheader("3. 우산 현황")
    st.write("**대여중인 우산**")
    st.write(", ".join(rented_umbrellas) if rented_umbrellas else "없음")
    st.write("**대여 가능한 우산**")
    st.write(", ".join(available_umbrellas) if available_umbrellas else "없음")
...
if not overdue_list:
    ...
    overdue_df["대여일"] = overdue_df["대여일"].apply(lambda x: x.strftime("%Y.%m.%d") if isinstance(x, date) else x)
    overdue_df["반납예정일"] = overdue_df["반납예정일"].apply(lambda x: x.strftime("%Y.%m.%d") if isinstance(x, date) else x)
    st.dataframe(overdue_df[["학번", "이름", "전화번호", "우산번호", "대여일", "반납예정일", "연체일수"]], use_container_width=True, hide_index=True)
    ...
    if not st.session_state.warning_logs:
        ...
        for column in ["대여일", "반납예정일", "경고일"]:
            if column in warning_df.columns:
                warning_df[column] = warning_df[column].apply(lambda x: x.strftime("%Y.%m.%d") if isinstance(x, date) else x)
        st.dataframe(warning_df[["학번", "이름", "전화번호", "우산번호", "대여일", "반납예정일", "경고일", "연체일수", "경고사유"]], use_container_width=True, hide_index=True)
...
if not st.session_state.return_logs:
    ...
    for column in ["대여일", "반납예정일", "반납처리일"]:
        if column in log_df.columns:
            log_df[column] = log_df[column].apply(lambda x: x.strftime("%Y.%m.%d") if isinstance(x, date) else x)
    if "경고부과" not in log_df.columns:
        log_df["경고부과"] = "아니오"
    if "연체일수" not in log_df.columns:
        log_df["연체일수"] = 0
    st.dataframe(log_df[["학번", "이름", "전화번호", "우산번호", "대여일", "반납예정일", "반납처리일", "연체일수", "경고부과"]], use_container_width=True, hide_index=True)
